...
class Countdown(tk.Frame): 
    ...
    def timer(self):
        ...
        if self.running:
            if self.time <= 0:
                self.clock.configure(text="TIMEOUT!")
            else:
                self.clock.configure(text=self.calculate())
                self.time -= 1
